...
async def main():
    ...
    trade_res = await client._request("GET", "https://fapi.binance.com/fapi/v1/userTrades", params={"symbol": "STABLEUSDT", "startTime": start_ts, "limit": 100}, signed=True)
    if trade_res.success:
        ...
        for t in trade_res.data:
            if t.get("positionSide") == "SHORT" and float(t.get("realizedPnl", 0)) != 0:
                print("SHORT PnL trade:", datetime.fromtimestamp(t.get("time")/1000, timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), t.get("side"), t.get("qty"), t.get("realizedPnl"))
